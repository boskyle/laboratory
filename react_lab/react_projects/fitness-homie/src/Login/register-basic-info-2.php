...
if (!$conn->connect_error) {
	...
	((SELECT userlogin_id FROM UserBasic WHERE userlogin_id = ?),?,?,?,?,?,?,?,?)")) {
        ...
        $stmt->bind_param("iiiiiiiss",$decoded['uid'],$decoded['age'],$decoded['height'],$decoded['weight'],$decoded['calories'],$decoded['calorieTarget'],$decoded['bmr'],$decoded['gender'],$decoded['activity_level']);
        $stmt->execute();
        $stmt->close();
        $conn->close();
    }
